...
- `src/patches.js` → shims contra bugs de builds do WhatsApp Web que o `whatsapp-web.js` ainda não trata. `applyPagePatches` roda a **cada** evento `ready` (um reload da página apaga a metade que vive nos protótipos do WhatsApp Web). Hoje só `patchSerializedIds`, portado do PR #150 de `avoylenko/wwebjs-api`: restaura `_serialized` em `Wid`/`MsgKey` nos builds 2.3000.x, que minificaram o campo. É no-op em build saudável.
...
- O download de mídia continua sendo o do `whatsapp-web.js` de estoque. Nos builds 2.3000.x ele falha (cai no `Msg.getMessagesById()`, que lança um `DataError` minificado, e re-descriptografa com chave velha); o fix é o PR #152 do upstream, deliberadamente adiado. Gatilho para portá-lo: anexo falhando com `t: t`, `InvalidMediaFileType` ou `hmac mismatch`, ou uma linha de log `Serialized id patch` com `msgKey: { patched: true }`.
